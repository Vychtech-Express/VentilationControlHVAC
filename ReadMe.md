...
Объемы доработок (вступительная часть):
 ...
 - эта формочка (пока еще макет) ведущая, все щиты - ведомые,
...
Примечания:
 ...
 - **OPC-сервер** ставится поверх **SCADA**, которая ставится на **Windows Server** с 4-м **.Net FrameWork**-ом, с **IIS** и вместе с **SQL Express** (но лучше полноценный внешний **SQL Server**) и которая не приобретается и не применяется, и функционал которого разрешается в USB-вом ключе за ежегодную дополнительную оплату
